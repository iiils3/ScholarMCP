...
// One-time beta repair: preserve courses/materials/deadlines, but discard outputs
// created by the old keyword prototype when they contain structural tokens.
try{
  ...
  if(!localStorage.getItem(marker)){
    ...
    if(raw){
      ...
      const bad=/(^|\b)(source|page|pdf|docx?|pptx?|file)(\b|$)/i;
      const corrupted=(state.topics||[]).some((x:any)=>bad.test(String(x?.title||''))) ||
        (state.flashcards||[]).some((x:any)=>bad.test(String(x?.topic||''))||bad.test(String(x?.front||''))) ||
        (state.artifacts||[]).some((x:any)=>JSON.stringify(x?.data||{}).match(/\b(source|page|pdf)\b/i));
      if(corrupted){
        state.topics=[];
        state.flashcards=[];
        state.artifacts=[];
