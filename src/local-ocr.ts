...
export async function structuredOCR(blob:Blob){
  ...
  return blobUrl(blob,async url=>{
    ...
    emit('أقرأ الصفحة وأحافظ على ترتيبها…',1);
    const ids:any=await model.generate({...inputs,max_new_tokens:2600,do_sample:false});
    const promptLength=(inputs.input_ids as any).dims?.at(-1)||0;
    let trimmed=ids;try{trimmed=ids.slice(null,[promptLength,null])}catch{}
    const decoded=processor.batch_decode(trimmed,{skip_special_tokens:true});
    return doctagsToText(String(decoded?.[0]||''));
  });
}
...
async function splitHandwrittenLines(blob:Blob){
  const bitmap=await createImageBitmap(blob);const scale=Math.min(1,1600/bitmap.width);const w=Math.max(1,Math.round(bitmap.width*scale)),h=Math.max(1,Math.round(bitmap.height*scale));
  const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d',{willReadFrequently:true})!;ctx.fillStyle='white';ctx.fillRect(0,0,w,h);ctx.drawImage(bitmap,0,0,w,h);bitmap.close();
  const img=ctx.getImageData(0,0,w,h).data;const active:boolean[]=[];
  for(let y=0;y<h;y++){let ink=0;for(let x=0;x<w;x+=3){const i=(y*w+x)*4;const gray=.299*img[i]+.587*img[i+1]+.114*img[i+2];if(gray<205)ink++}active[y]=ink>Math.max(2,w/130)}
  const ranges:Array<[number,number]>=[];let start=-1,gap=0;
  for(let y=0;y<h;y++){if(active[y]){if(start<0)start=y;gap=0}else if(start>=0){gap++;if(gap>7){const end=y-gap;if(end-start>7)ranges.push([Math.max(0,start-5),Math.min(h,end+6)]);start=-1;gap=0}}}
  if(start>=0)ranges.push([Math.max(0,start-5),h]);
  const usable=ranges.filter(([a,b])=>b-a>9).slice(0,80);if(!usable.length)return [blob];
  const out:Blob[]=[];for(const [a,b] of usable){const line=document.createElement('canvas');line.width=w;line.height=b-a;line.getContext('2d')!.drawImage(c,0,a,w,b-a,0,0,w,b-a);out.push(await canvasToBlob(line))}return out;
}

async function getTrocr(){
  if(trocrPromise)return trocrPromise;
  trocrPromise=(async()=>{const {pipeline}=await import('@huggingface/transformers');emit('جاري تجهيز قارئ الخط اليدوي…',0.05);const p=await pipeline('image-to-text','Xenova/trocr-small-handwritten',{device:webgpu()?'webgpu':'wasm',dtype:webgpu()?'fp16':'q8',progress_callback:(x:any)=>emit('جاري تنزيل قارئ الخط اليدوي لأول مرة…',typeof x?.progress==='number'?x.progress/100:undefined)} as any);emit('قارئ الخط اليدوي جاهز',1);return p})().catch(e=>{trocrPromise=null;throw e});
  return trocrPromise;
}
...
export async function ocrSource(blob:Blob,mode:'auto'|'document'|'handwriting'='auto'){
  if(mode==='handwriting'){
    try{const hand=await handwrittenOCR(blob);if(hand.length>12)return hand}catch{}
    return structuredOCR(blob);
  }
  const structured=await structuredOCR(blob);
  if(mode==='document'||structured.length>35)return structured;
  try{const hand=await handwrittenOCR(blob);return hand.length>structured.length?hand:structured}catch{return structured}
}
