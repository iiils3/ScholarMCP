...
function fallbackCustom(prompt:string,context:string){
  ...
  if(p.includes('supported|partial|unsupported|contradicted')){
    const claim=(prompt.match(/«([^»]+)»/)||[])[1]||'';const terms=claim.toLowerCase().match(/[\p{L}\p{N}]{4,}/gu)||[];const hit=ss.find(s=>terms.filter(t=>s.toLowerCase().includes(t)).length>=Math.max(1,Math.min(2,terms.length)));
    return {status:hit?'partial':'unsupported',confidence:hit?62:30,explanation:hit?'وجدت سياقًا مرتبطًا بالادعاء، لكن الفحص الخفيف لا يثبت التطابق الكامل.':'لم أجد دعمًا واضحًا للادعاء في المقاطع المحلية المتاحة.',evidence:hit?[{quote:hit.slice(0,220),sourceRef:sourceRefs[0]}]:[],missing:hit?'شغّل Scholar AI الكامل لفحص دلالي أدق.':'أضف مصدرًا يحتوي المعلومة أو عدّل الادعاء.'};
  }
  if(p.includes('literature review')||p.includes('"synthesis"')&&p.includes('"themes"')){
    const paperTitles=[...context.matchAll(/^Title:\s*(.+)$/gmi)].map(m=>m[1].trim()).slice(0,12);return {synthesis:`المراجعة تضم ${paperTitles.length} أوراق محددة. المحرك الخفيف يحافظ على حدود البيانات ولا يدعي قراءة Full Text عندما المتاح Abstract فقط.`,themes:paperTitles.slice(0,5).map((title,i)=>({theme:`محور ${i+1}`,papers:[i+1],finding:title})),agreements:[],disagreements:[],gaps:['شغّل Scholar AI المحلي الكامل لاستخراج ثيمات دلالية أعمق.'],limitations:['التحليل الاحتياطي يعتمد على البيانات الوصفية والملخصات المتاحة فقط.'],nextQuestions:[]};
  }
  if(p.includes('integrity guard')||p.includes('"closeparaphrase"')){
    const hasRefs=/—\s*ص\s*\d+|\([^)]*ص\s*\d+[^)]*\)/.test(prompt);return {score:hasRefs?78:55,unsupported:hasRefs?[]:[{text:'المسودة تحتاج إحالات أوضح للمصدر.',reason:'لم أجد إشارات صفحات واضحة في الفحص الخفيف.',sourceRef:sourceRefs[0]}],citationIssues:hasRefs?[]:['أضف استشهادًا قابلًا للتتبع عند الادعاءات الأساسية.'],closeParaphrase:[],inventedReferences:[],safe:['لم يدّعِ الفحص الاحتياطي كشف سرقة أدبية من الإنترنت.'],verdict:hasRefs?'البنية مقبولة مبدئيًا وتحتاج مراجعة دلالية نهائية.':'لا تسلّم قبل ربط الادعاءات بالمصادر.'};
  }
...
export async function smartTask(action:string,payload:any){
  ...
  try{const data=await runSmartTask(action,payload);lastStatus={stage:'local-ai',progress:100,message:'اكتملت المعالجة محليًا'};return data}
  catch(e){lastStatus={stage:'local-ai',progress:0,message:'استخدمت المعالجة المحلية الخفيفة'};throw e}
}
export async function customTask(prompt:string,context=''){
  ...
  try{const data=await runCustomTask(prompt,context);lastStatus={stage:'local-ai',progress:100,message:'اكتملت المهمة الأكاديمية محليًا'};return data}
  catch(e){console.warn('ScholarMCP local model fallback:',e);lastStatus={stage:'fallback',progress:100,message:'تمت المهمة بمحرك Scholar الخفيف'};return fallbackCustom(prompt,context)}
}
export async function ocrSource(source:File|Blob|string){const blob=await sourceBlob(source);lastStatus={stage:'ocr',progress:15,message:'Scholar OCR يقرأ الصفحة على جهازك'};const text=await runOCR(blob,'auto');lastStatus={stage:'ocr',progress:100,message:'اكتملت قراءة الصفحة محليًا'};return text.trim()}
export async function structuredOCR(source:File|Blob|string){const blob=await sourceBlob(source);const text=await runStructuredOCR(blob);return {text:text.trim(),provider:'scholarmcp-local',handwriting:true,local:true}}
export async function handwrittenOCR(source:File|Blob|string){const blob=await sourceBlob(source);return (await runHandwrittenOCR(blob)).trim()}
...
export async function speechToText(source:File|Blob|string,_options:any={}){lastStatus={stage:'speech',progress:10,message:'ScholarMCP يفرّغ المحاضرة على جهازك'};const result=await transcribeAudio(source);lastStatus={stage:'speech',progress:100,message:'اكتمل تفريغ المحاضرة محليًا'};return result}
export async function textToSpeech(text:string,options:any={}){if(!('speechSynthesis'in window))throw new Error('القراءة الصوتية غير مدعومة على هذا الجهاز.');window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(text.slice(0,9000));utterance.lang=options.lang||(/[\u0600-\u06ff]/.test(text)?'ar-IQ':'en-US');utterance.rate=Number(options.rate)||.95;window.speechSynthesis.speak(utterance);return true}
export async function generateStudyImage(_prompt:string,_options:any={}){throw new Error('الرسوم التوليدية الثقيلة غير مفعلة في النسخة المحلية؛ استخدم الخريطة الذهنية أو فيديو Scholar المحلي.')}
export async function generateStudyVideo(_prompt:string,_options:any={}){throw new Error('استخدم استوديو فيديو AI داخل المركز الأكاديمي؛ التوليد المرئي يتم محليًا من المادة.')}
export async function localAIStatus(){const ai=await getAIStatus();return {...lastStatus,...ai,deviceCompute:true,provider:'scholarmcp-local',speech:speechCapabilities(),ocr:getOCRCapabilities()}}
