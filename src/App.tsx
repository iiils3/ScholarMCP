...
function Study({state,notify}:{state:AppState;notify:(m:string)=>void}){
  ...
  async function build(){if(!course||!mats.length){notify('اختر مادة فيها مصادر أولًا');return}setBusy(true);const context=mats.map(m=>`SOURCE ${m.name}\n${m.text}`).join('\n\n').slice(0,70000);const r:any=await aiTask('study',{context,mode,weakTopics:weak.map(x=>x.title),daysToExam:daysUntil(course.examDate)});setSteps((r.steps||[]).map((x:any)=>({...x,done:false})));setBusy(false)}
...
function Assignments({state,notify}:{state:AppState;notify:(m:string)=>void}){
  ...
  async function run(){if(!courseId||!instructions.trim())return;const mats=state.materials.filter(m=>m.courseId===courseId&&m.status==='ready');setBusy(true);const context=mats.map(m=>`SOURCE ${m.name}\n${m.text}`).join('\n\n').slice(0,80000);const r:any=await aiTask('assignment',{context,instructions,rubric});setResult(r);repo.addAssignment({courseId,title:title||'واجب أكاديمي',instructions,rubric,dueAt:due||undefined,checklist:Array.isArray(r.checklist)?r.checklist:[],outline:typeof r.outline==='string'?r.outline:Array.isArray(r.outline)?r.outline.join('\n'):undefined,draft:typeof r.draft==='string'?r.draft:undefined,assessment:r.assessment});notify('تم حفظ تحليل الواجب');setBusy(false)}
